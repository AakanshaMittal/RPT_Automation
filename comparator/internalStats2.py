...
def compare_tables(t1, t2, tol=1e-5):
    mismatches = []
...
<table><tr><th>Table</th><th>Mismatches</th></tr>
"""
...
def generate_consolidated_excel(summary, all_results, output_path):
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        # TC sheets -> ONLY tables with mismatches
        for tc, results in all_results.items():
            ...
            for name, (t1, t2, mm) in results.items():
                ...
                t1.to_excel(writer, sheet_name=sheet, startrow=row + 2,
                            startcol=0, index=False, header=False)

                offset = t1.shape[1] + 3
                t2.to_excel(writer, sheet_name=sheet, startrow=row + 2,
                            startcol=offset, index=False, header=False)
